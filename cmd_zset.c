/*
 * cmd_zset.c  –  Sorted Set 커맨드 구현 (s_replyObject 반환)
 */
#define _GNU_SOURCE
#include <stdlib.h>
#include <errno.h>
...
/* ── 내부: 키 타입 검사 (버킷 잠긴 상태) ────────────────────
 *   1=새 키  0=기존 ZSET  SHM_ERR_KEY_EXISTS=다른 타입
 * ─────────────────────────────────────────────────────────── */
...
/* ── 내부: ZSetHeader + HEAD 노드 생성 (버킷 잠긴 상태) ───── */
                                ...
                                const void *key, uint32_t klen)
{
    ...
    pthread_mutex_init(&zsh->mutex, &ma); pthread_mutexattr_destroy(&ma);

    uint64_t hd_off = heap_alloc(h, sizeof(SkipNode));
    if (hd_off == OFFSET_NULL) { heap_free(h, zsh_off); return SHM_ERR_NOMEM; }
    SkipNode *hd = core_sn(h, hd_off);
    hd->score = -__builtin_inf(); hd->member_offset = OFFSET_NULL;
    hd->member_len = 0; hd->level_count = ZSET_MAX_LEVEL; hd->backward_offset = OFFSET_NULL;
    ...
    if (ne_off == OFFSET_NULL) { heap_free(h, hd_off); heap_free(h, zsh_off); return SHM_ERR_NOMEM; }
    ((NameEntry *)OFF2PTR(h, ne_off))->next_offset = bk->head_offset;
    bk->head_offset = ne_off;
    return SHM_OK;
}

/* ── 내부: ZSet 얻기, 없으면 자동 생성 ─────────────────────── */
                                       ...
                                       int *err)
{
    ...
    if (chk == 0) {
        ZSetHeader *z = (ZSetHeader *)OFF2PTR(h, ((NameEntry *)OFF2PTR(h, ne_off))->data_offset);
        pthread_mutex_unlock(&bk->mutex); if (err) *err = SHM_OK; return z;
    }
...
s_replyObject *cmd_zdrop(ShmHandle *h, string_t *args[], uint32_t argc)
{
    ...
    pthread_mutex_unlock(&bk->mutex);
    pthread_mutex_lock(&zsh->mutex);
    uint64_t cur = core_sn(h, zsh->head_offset)->forward[0];
    while (cur != OFFSET_NULL) { uint64_t nx = core_sn(h,cur)->forward[0]; sl_node_free(h,cur); cur=nx; }
...
/* ── ZADD ────────────────────────────────────────────────────
 *  args: ZADD key [NX|XX|GT|LT|CH] score member [score member…]
 *  반환: INTEGER (추가/변경 수)
 * ─────────────────────────────────────────────────────────── */
s_replyObject *cmd_zadd(ShmHandle *h, string_t *args[], uint32_t argc)
{
    ...
    for (uint32_t i = pos; i + 1 < argc; i += 2) {
        char *ep = NULL; double score = strtod(args[i]->ptr, &ep);
        if (ep == args[i]->ptr) { pthread_mutex_unlock(&zsh->mutex); return reply_error(SHM_ERR_PARSE,"score 변환 실패"); }
        const void *member = args[i+1]->ptr; uint32_t mlen = args[i+1]->len;
        if (!mlen) { pthread_mutex_unlock(&zsh->mutex); return reply_error(SHM_ERR_INVAL,"member 비어있음"); }
        ...
        if (existing) {
            ...
            if (doit && score != old) {
                sl_find_update(h,zsh,old,OFF2PTR(h,existing->member_offset),existing->member_len,update);
                sl_unlink(h,zsh,exist_off,update);
                sl_find_update(h,zsh,score,member,mlen,update);
                uint32_t lv = sl_random_level();
                if (lv > zsh->cur_level) { for(uint32_t li=zsh->cur_level;li<lv;li++) update[li]=zsh->head_offset; zsh->cur_level=lv; }
                existing->score = score; existing->level_count = lv;
                for (uint32_t li=0;li<lv;li++) { existing->forward[li]=core_sn(h,update[li])->forward[li]; core_sn(h,update[li])->forward[li]=exist_off; }
                for (uint32_t li=lv;li<ZSET_MAX_LEVEL;li++) existing->forward[li]=OFFSET_NULL;
                if (existing->forward[0]!=OFFSET_NULL) core_sn(h,existing->forward[0])->backward_offset=exist_off;
                else zsh->tail_offset=exist_off;
                existing->backward_offset=update[0]; total_changed++;
            }
        } else {
            ...
            if (n==OFFSET_NULL) { pthread_mutex_unlock(&zsh->mutex); return reply_error(SHM_ERR_NOMEM,shm_strerror(SHM_ERR_NOMEM)); }
            SkipNode *sn=core_sn(h,n);
            for(uint32_t li=0;li<lv;li++){ sn->forward[li]=core_sn(h,update[li])->forward[li]; core_sn(h,update[li])->forward[li]=n; }
            sn->backward_offset=update[0];
            if(sn->forward[0]!=OFFSET_NULL) core_sn(h,sn->forward[0])->backward_offset=n; else zsh->tail_offset=n;
            zsh->length++; total_added++;
        }
    }
    pthread_mutex_unlock(&zsh->mutex);
...
s_replyObject *cmd_zrem(ShmHandle *h, string_t *args[], uint32_t argc)
{
    ...
    for (uint32_t i = 2; i < argc; i++) {
        ...
        uint64_t upd[ZSET_MAX_LEVEL]; SkipNode *sn = core_sn(h,n);
        sl_find_update(h,zsh,sn->score,OFF2PTR(h,sn->member_offset),sn->member_len,upd);
        sl_unlink(h,zsh,n,upd); sl_node_free(h,n); zsh->length--; removed++;
    }
    pthread_mutex_unlock(&zsh->mutex);
...
s_replyObject *cmd_zscore(ShmHandle *h, string_t *args[], uint32_t argc)
{
    ...
    pthread_mutex_unlock(&zsh->mutex);
    /* score 를 문자열 표현으로 STRING 반환 (Redis 호환) */
...
s_replyObject *cmd_zincrby(ShmHandle *h, string_t *args[], uint32_t argc)
{
    ...
    char *ep = NULL; double delta = strtod(args[2]->ptr, &ep);
    ...
    if (zsh) {
        int rc = pthread_mutex_lock(&zsh->mutex);
        if (rc == EOWNERDEAD) pthread_mutex_consistent(&zsh->mutex);
        uint64_t n = sl_find_member(h, zsh, member, mlen);
        if (n != OFFSET_NULL) new_sc = core_sn(h,n)->score + delta;
        pthread_mutex_unlock(&zsh->mutex);
    }
    /* ZADD 로 재삽입 */
    char sbuf[64]; int sl = snprintf(sbuf, sizeof(sbuf), "%.17g", new_sc);
    string_t a0=STR_LIT("ZADD"), a2={.ptr=sbuf,.len=(uint32_t)sl};
    string_t *sub[4] = { &a0, args[1], &a2, args[3] };
    s_replyObject *r = cmd_zadd(h, sub, 4);
    reply_free(r);
...
/* ── ZRANK / ZREVRANK ────────────────────────────────────── */
static s_replyObject *zrank_impl(ShmHandle *h, string_t *args[], uint32_t argc, int rev)
{
    if (argc < 3) return reply_error(SHM_ERR_ARGC, rev?"usage: ZREVRANK key member":"usage: ZRANK key member");
    ZSetHeader *zsh = core_zset_get(h, args[1]->ptr, args[1]->len);
    if (!zsh) return reply_nil();
    int rc = pthread_mutex_lock(&zsh->mutex);
    if (rc == EOWNERDEAD) pthread_mutex_consistent(&zsh->mutex);
    ...
    while (cur != OFFSET_NULL) {
        ...
        if (sn->member_len == args[2]->len && !memcmp(OFF2PTR(h,sn->member_offset),args[2]->ptr,args[2]->len)) { found = rank; break; }
...
s_replyObject *cmd_zcount(ShmHandle *h, string_t *args[], uint32_t argc)
{
    ...
    int rc = pthread_mutex_lock(&zsh->mutex);
    if (rc == EOWNERDEAD) pthread_mutex_consistent(&zsh->mutex);
    int64_t cnt = 0; uint64_t cur = core_sn(h,zsh->head_offset)->forward[0];
    while (cur != OFFSET_NULL) { SkipNode *sn=core_sn(h,cur); if(sn->score>mx)break; if(sn->score>=mn)cnt++; cur=sn->forward[0]; }
    pthread_mutex_unlock(&zsh->mutex);
    return reply_integer(cnt);
}

/* ── 내부: 범위 탐색 결과를 ARRAY(member, score 쌍) 로 변환 ── */
                                          ...
                                          uint64_t *nodes, int64_t count)
{
    ...
    for (int64_t i = 0; i < count; i++) {
        ...
        s_replyObject *mem = reply_string((const char *)OFF2PTR(h, sn->member_offset), sn->member_len);
...
/* ── ZRANGE ──────────────────────────────────────────────────
 *  args: ZRANGE key start stop [REV]
 *  반환: ARRAY [member score member score …]
 * ─────────────────────────────────────────────────────────── */
s_replyObject *cmd_zrange(ShmHandle *h, string_t *args[], uint32_t argc)
{
    ...
    if (!rev) {
        int64_t i = 0; uint64_t cur = core_sn(h, zsh->head_offset)->forward[0];
        while (cur != OFFSET_NULL && i <= stop) { if (i >= start) nodes[filled++] = cur; cur = core_sn(h,cur)->forward[0]; i++; }
    } else {
...
/* ── ZRANGEBYSCORE ───────────────────────────────────────────
 *  args: ZRANGEBYSCORE key min max [REV] [LIMIT offset count]
 * ─────────────────────────────────────────────────────────── */
s_replyObject *cmd_zrangebyscore(ShmHandle *h, string_t *args[], uint32_t argc)
{
    if (argc < 4) return reply_error(SHM_ERR_ARGC,"usage: ZRANGEBYSCORE key min max [REV] [LIMIT offset count]");
    double mn = strtod(args[2]->ptr,NULL), mx = strtod(args[3]->ptr,NULL);
    ...
    for (uint32_t i = 4; i < argc; i++) {
        if (!strcasecmp(args[i]->ptr,"REV")) rev = 1;
            ...
            loff = strtoll(args[i+1]->ptr,NULL,10); lcnt = strtoll(args[i+2]->ptr,NULL,10); i += 2;
        }
    }
    ZSetHeader *zsh = core_zset_get(h, args[1]->ptr, args[1]->len);
    ...
    if (!rev) {
        uint64_t cur = core_sn(h,zsh->head_offset)->forward[0];
        while (cur != OFFSET_NULL) {
            SkipNode *sn = core_sn(h,cur); if(sn->score>mx)break;
            if (sn->score >= mn) {
                if (skipped < loff){skipped++;}
                else{ if(lcnt>=0&&filled>=lcnt)break; if(filled>=cap){cap*=2;uint64_t*t=(uint64_t*)realloc(nodes,(size_t)cap*sizeof(uint64_t));if(!t){free(nodes);pthread_mutex_unlock(&zsh->mutex);return reply_error(SHM_ERR_NOMEM,shm_strerror(SHM_ERR_NOMEM));}nodes=t;} nodes[filled++]=cur; }
            }
            cur=sn->forward[0];
        }
    } else {
        uint64_t cur = zsh->tail_offset;
        while (cur != OFFSET_NULL) {
            SkipNode *sn=core_sn(h,cur); if(sn->score<mn)break;
            if (sn->score <= mx) {
                if(skipped<loff){skipped++;}
                else{ if(lcnt>=0&&filled>=lcnt)break; if(filled>=cap){cap*=2;uint64_t*t=(uint64_t*)realloc(nodes,(size_t)cap*sizeof(uint64_t));if(!t){free(nodes);pthread_mutex_unlock(&zsh->mutex);return reply_error(SHM_ERR_NOMEM,shm_strerror(SHM_ERR_NOMEM));}nodes=t;} nodes[filled++]=cur; }
            }
 ...
 * ─────────────────────────────────────────────────────────── */
static s_replyObject *zpop_impl(ShmHandle *h, string_t *args[], uint32_t argc, int from_min)
{
    if (argc < 2) return reply_error(SHM_ERR_ARGC, from_min ? "usage: ZPOPMIN key [count]" : "usage: ZPOPMAX key [count]");
    int64_t count = 1;
    ...
    for (int64_t i = 0; i < count && zsh->length > 0; i++) {
        uint64_t n = from_min ? core_sn(h,zsh->head_offset)->forward[0] : zsh->tail_offset;
        ...
        sl_find_update(h,zsh,sn->score,OFF2PTR(h,sn->member_offset),sn->member_len,upd);
        sl_unlink(h,zsh,n,upd); sl_node_free(h,n); zsh->length--;
    }
    pthread_mutex_unlock(&zsh->mutex);
    return arr;
}
